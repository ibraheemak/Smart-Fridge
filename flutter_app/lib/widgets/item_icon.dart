...
/// 4. Open Food Facts product photo (by barcode) — free, no API key/quota
/// 5. Gemini AI generation                      — for new items (needs quota)
/// 6. Coloured letter avatar                    — last resort
class ItemIcon extends StatefulWidget {
  final String itemName;

  /// EAN barcode, if the item was added by the scanner. Enables a free
  /// Open Food Facts product photo when no designed icon exists.
...
class _ItemIconState extends State<ItemIcon> {
  ...
  /// Open Food Facts product-photo URL (resolved from the barcode)
  String? _offUrl;
  ...
  void didUpdateWidget(ItemIcon old) {
    ...
    if (old.itemName != widget.itemName || old.barcode != widget.barcode) {
      setState(() {
        _asset = null;
        _assetChecked = false;
        _offUrl = null;
  ...
  Future<void> _loadOffPhoto() async {
    if (widget.barcode.isEmpty) return;
    final url = await OpenFoodFactsService.imageForBarcode(widget.barcode);
    if (mounted && url != null) setState(() => _offUrl = url);
  }
  ...
  Widget _buildContent() {
    ...
    return CachedNetworkImage(
      ...
      errorWidget: (_, __, ___) =>
        ...
        errorWidget: (_, __, ___) {
          // Tier 4: Open Food Facts product photo (free, by barcode).
          if (_offUrl != null) {
            return CachedNetworkImage(
              imageUrl: _offUrl!,
              fit: BoxFit.cover,
  ...
  /// Tier 5 (Gemini generation, if quota) → Tier 6 (letter avatar).
  Widget _fallback() {
